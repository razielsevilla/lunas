...
export function FAQTestimonials() {
  ...
  return (
    <section className="relative overflow-hidden bg-ivory py-32 text-night">
      ...
      <div className="relative mx-auto max-w-6xl px-6">
        ...
        <div className="mb-20 flex flex-col items-start justify-between gap-8 md:flex-row md:items-end">
          ...
          <button className="rounded-full border border-border bg-white px-8 py-3 text-sm font-semibold text-night transition-all hover:border-golden/40 hover:shadow-soft">
            Contact support
          </button>
        </div>
          ...
          <div className="space-y-4">
            {faqs.map((faq, index) => (
              <div 
                ...
                className={`group cursor-pointer rounded-3xl border border-border p-8 transition-all duration-300 ${
                  openIndex === index ? "bg-white border-golden/30 shadow-soft" : "bg-transparent hover:border-golden/50"
                }`}
              >
                ...
                <div className="mt-4 flex items-center justify-between gap-4">
                  <h3 className={`text-xl font-medium tracking-tight transition-colors ${
                    openIndex === index ? "text-night" : "text-night/80 group-hover:text-night"
                  }`}>
                    {faq.question}
                  </h3>
                  <div className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full border border-border text-night/30 transition-all duration-500 ${
                    openIndex === index ? "rotate-45 border-golden bg-golden/5 text-golden" : "group-hover:border-golden/40 group-hover:text-golden"
                  }`}>
          ...
          <div className="space-y-12">
            <div className="space-y-6">
              <div className="inline-flex items-center gap-2.5 rounded-full border border-border bg-white px-4 py-1.5 shadow-soft">
                ...
                <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-muted-foreground/60">Testimonials</span>
              </div>
            ...
            <div className="space-y-6">
              {testimonials.map((t, index) => (
                <div 
                  key={index} 
                  className="relative overflow-hidden rounded-[2rem] border border-border/50 bg-white p-8 transition-all duration-500 hover:border-golden/20 hover:shadow-soft-lg group"
                >
